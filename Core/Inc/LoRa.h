...
#define RegPktRssiValue				0x1A   //readRegister(REG_PKT_RSSI_VALUE) - (_frequency < RF_MID_BAND_THRESHOLD ? RSSI_OFFSET_LF_PORT : RSSI_OFFSET_HF_PORT))
...
void LoRa_getResponsePing(LoRa* _LoRa,uint8_t* data, uint8_t length);

void LoRa_addNeighbor(int neighbor_id, const int rssi);
LoRa_Neighbor *LoRa_findNeighbor(int neighbor_id);
void LoRa_deleteNeighbor(LoRa_Neighbor *neighbor);
void LoRa_deleteAll(void);
void LoRa_printNeighbors(void);
int by_rssi(const LoRa_Neighbor *a, const LoRa_Neighbor *b);
