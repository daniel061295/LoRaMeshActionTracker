...
//	uint8_t buff[128];
	LoRa_receive(_LoRa, data, length);
}
...
LoRa_Neighbor *LoRa_findNeighbor(int neighbor_id)
{
    ...
    HASH_FIND_INT(neighbors, &neighbor_id, s);  /* s: output pointer */
    return s;
}
...
void LoRa_printNeighbors()
{
    LoRa_Neighbor *s;

    for (s = neighbors; s != NULL; s = (LoRa_Neighbor*)(s->hh.next)) {
        printf("neighbor id %d: rssi %d\n", s->id, s->rssi);
    }
}
